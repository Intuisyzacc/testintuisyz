...
if ($conn->connect_error) {
    ...
    $stmt = $conn->prepare("INSERT INTO `enquiry_index` (id, name,website, email, phone,msg, date, time, status, service,) VALUES (null, ?, ?, ?, ?,?, CURDATE(), CURTIME(), ?,?)");

    // Use "sssss" if all values are strings, otherwise, adjust the types accordingly
    $stmt->bind_param("sssss", $name, $email, $phone, $mg,$status,$service);
